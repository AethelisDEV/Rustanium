...
pub static TERM_ROW: AtomicUsize = AtomicUsize::new(0);
pub static TERM_COL: AtomicUsize = AtomicUsize::new(0);

pub fn term_newline() {
    let row = TERM_ROW.load(Ordering::Relaxed);
    TERM_COL.store(0, Ordering::Relaxed);
    if row < TERM_MAX_ROWS - 1 {
        TERM_ROW.store(row + 1, Ordering::Relaxed);
    } else {
        unsafe {
...
pub fn term_print_char(c: char) {
    if c == '\n' {
        ...
        unsafe {
            for c_idx in 0..col {
                if c_idx + 1 < col && TERM_BUFFER[row][c_idx] == '>' && TERM_BUFFER[row][c_idx + 1] == ' ' {
                    prompt_end = c_idx + 2;
                }
            }
            if col > prompt_end {
                let new_col = col - 1;
                TERM_COL.store(new_col, Ordering::Relaxed);
                TERM_BUFFER[row][new_col] = ' ';
            }
        }
    } else {
        let col = TERM_COL.load(Ordering::Relaxed);
        ...
        if col < TERM_MAX_COLS {
            unsafe {
                TERM_BUFFER[row][col] = c;
            }
            TERM_COL.store(col + 1, Ordering::Relaxed);
        }
        if TERM_COL.load(Ordering::Relaxed) >= TERM_MAX_COLS {
            term_newline();
        }
    }
}
...
pub fn term_process_command() {
    ...
    if start_col == 0 {
        start_col = if col > 11 { 11 } else { col };
    }

    if col > start_col {
        unsafe {
            for c_idx in start_col..col {
                if line_len < 128 {
                    line_buf[line_len] = TERM_BUFFER[row][c_idx] as u8;
                    line_len += 1;
                }
            }
        }
    ...
    if cmd_str.eq_ignore_ascii_case("help") {
        ...
        TERM_ROW.store(0, Ordering::Relaxed);
        TERM_COL.store(0, Ordering::Relaxed);
        ...
        if path.is_empty() {
            ...
            if fd == u64::MAX || fd >= 16 {
                ...
                if bytes_read == u64::MAX {
                    ...
                    let slice = unsafe { core::slice::from_raw_parts(io_buf_ptr as *const u8, bytes_read as usize) };
                    if let Ok(content_str) = core::str::from_utf8(slice) {
                        term_print_str(content_str);
